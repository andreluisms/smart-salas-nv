...
BLEServerService* __bleClientconfig; 
        ...
        while (client.connected()) {
          ...
          if (client.available()) {
            ...
            if (request.type == CONDICIONADOR || request.type == LUZES) { 
                ...
                //__bleClientconfig->setReceivedRequest(true);
                ...
                if(dispConnected)
                {
                  ...
                  __bleClientconfig->sendMessageToActuator(payload);

                  awaitsReturn();

                  __bleClientconfig->disconnectToActuator();
                }
                
                //__bleClientconfig->setReceivedRequest(false);
...
bool ClientSocketService::connectToActuator(String uuidDevice) 
{
  ...
  { 
    ...
    deviceConnected = __bleClientconfig->connectToActuator(uuidDevice);
