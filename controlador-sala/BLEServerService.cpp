...
std::vector<BLEAdvertisedDevice*> BLEServerService::__filteredDevices;
std::unordered_map<std::string, Hardware> BLEServerService::__devicesMapped;
...
{  
  ...
  while (true)
  {
    ...
    if(!HTTP_REQUEST && !ENV_REQUEST)
    {
      ...
      config.unlockEnvVariablesMutex();
    }
  }
}
...
void BLEServerService::newCicle()
{
    ...
    std::vector<BLEDeviceConnect*> aux;
...
void BLEServerService::closeConnections(std::vector<BLEDeviceConnect*> aux)
{
...
std::vector<String> BLEServerService::getSensors()
{
  return __sensors;
}

std::vector<struct HardwareRecord> BLEServerService::getActuators()
{
  return __actuators;
}
